...
// 새로운 access token을 발급받는 비동기 방식의 요청을 모듈화. (fetch는 interceptor 기능x)
// axios 인스턴스는 token이 필요한 모든 요청에 활용 될 것입니다.

import axios from 'axios';
import { API_BASE_URL as BASE } from './host-config';
...
요청을 수정하거나, 응답에 대한 결과 처리를 수행 할 수 있습니다.
*/

// Request Interceptor 설정
axiosIntance.interceptors.request.use(
  // 요청 보내기 전에 일괄 처리해야 할 내용을 함수로 선언. // 결과처리 함수
  (config) => {
  ...
  (error) => Promise.reject(error), // 에러 발생할때 처리 할 함수
);
...
axiosIntance.interceptors.response.use(
  ...
  async (error) => {
    console.log(
      'response Intercepter가 동작! 응답 에러 발생!',
    );
    // 원본 요청의 정보를 기억 해 놓자 -> 새 토큰 발급 받아서 다시 보낼 꺼니까.
    ...
    if (
      ...
      try {
        ...
        if (res.status === 200) {
          ...
          const { accessToken } = res.data; // axios는 json()함수 한씁니다. data라고 하면 json 객체 바로 리턴.
          ...
          // axiosIntance를 사용해서 다시 한번 원본의 요청을 보낼 거고, 응답값을 원래 호출한 곳으로 리턴.
